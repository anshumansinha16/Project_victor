...
class Encoder(nn.Module):
    def __init__(self, input_dim1, hidden_dim1, input_dim2, hidden_dim2, input_dim3, hidden_dim3, hidden_layers, y_dim, latent_dim, act_type, act_args):
        super().__init__()

        self.hidden1 = nn.Sequential()
        dims_arr1 = np.linspace(hidden_dim1, latent_dim, hidden_layers+1, dtype=int)[0:-1]
        dims_arr1 = np.concatenate(([input_dim1 + y_dim], dims_arr1))
        
        for i, (in_size, out_size) in enumerate(zip(dims_arr1[:-1], dims_arr1[1:])):
            self.hidden1.add_module(
                name='Linear_'+str(i),
        ...
        dims_arr2 = np.linspace(hidden_dim2, latent_dim, hidden_layers+1, dtype=int)[0:-1]
        dims_arr2 = np.concatenate(([input_dim2 + y_dim], dims_arr2))
        
        for i, (in_size, out_size) in enumerate(zip(dims_arr2[:-1], dims_arr2[1:])):
            self.hidden2.add_module(
                name='Linear_'+str(i),
                module= nn.Linear(in_size, out_size)
            )
            print('in_size = ', in_size)
        ...
        dims_arr3 = np.linspace(hidden_dim3, latent_dim, hidden_layers+1, dtype=int)[0:-1]
        dims_arr3 = np.concatenate(([input_dim3 + y_dim], dims_arr3))
        
        for i, (in_size, out_size) in enumerate(zip(dims_arr3[:-1], dims_arr3[1:])):
            self.hidden3.add_module(
                name='Linear_'+str(i),
                module= nn.Linear(in_size, out_size)
            )
            print('in_size = ', in_size)
        ...
        self.mu = nn.Linear(dims_arr1[-1]+dims_arr2[-1]+dims_arr3[-1], latent_dim)
        self.var = nn.Linear(dims_arr1[-1]+dims_arr2[-1]+dims_arr3[-1], latent_dim)

    def forward(self, x):
        x1,x2,x3,y = x[:,:600],x[:,600:701],x[:,701:708],x[:,708]
        y = y.view(-1,1)

        x1 = torch.cat((x1,y),dim=1)
        x2 = torch.cat((x2,y),dim=1)
        x3 = torch.cat((x3,y),dim=1)

        x1,x2,x3 = self.hidden1(x1), self.hidden2(x2), self.hidden3(x3)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim1, hidden_dim2, hidden_dim3, output_dim, hidden_layers, y_dim, act_type, act_args):
        super().__init__()

        self.hidden1 = nn.Sequential()
        dims_arr1 = np.linspace(latent_dim + y_dim, hidden_dim1, hidden_layers+1, dtype=int)
        ...
        self.hidden2 = nn.Sequential()
        dims_arr2 = np.linspace(latent_dim + y_dim, hidden_dim2, hidden_layers+1, dtype=int)
        ...
        self.hidden3 = nn.Sequential()
        dims_arr3 = np.linspace(latent_dim + y_dim, hidden_dim3, hidden_layers+1, dtype=int)
        ...
        h_dim = hidden_dim1 + hidden_dim2 + hidden_dim3
        print('h_dim',h_dim)
        print('output_dim',output_dim)
        self.hidden_to_out = nn.Linear(h_dim, output_dim)
...
class CVAE(nn.Module):
    def __init__(self, output_dim, input_dim1, hidden_dim1, input_dim2, hidden_dim2, input_dim3, hidden_dim3, hidden_layers, y_dim, latent_dim, act_type, act_args={}):
        super().__init__()
        self.encoder = Encoder(input_dim1, hidden_dim1, input_dim2, hidden_dim2, input_dim3, hidden_dim3, hidden_layers, y_dim, latent_dim, act_type, act_args) #self, input_dim1, hidden_dim1, input_dim2, hidden_dim2, input_dim3, hidden_dim3, hidden_layers, y_dim, latent_dim, act_type, act_args
        self.decoder = Decoder(latent_dim, hidden_dim1, hidden_dim2, hidden_dim3, output_dim, hidden_layers, y_dim, act_type, act_args)
...
CONFIG['params']['input_dim2'] = 101 # input dimmension
CONFIG['params']['input_dim3'] = 7 # input dimmension

CONFIG['params']['hidden_dim'] = 512
CONFIG['params']['hidden_dim1'] = 512
CONFIG['params']['hidden_dim2'] = 64
CONFIG['params']['hidden_dim3'] = 8
...
class Trainer():
    ...
    def initialize(self):
        ...
        # create model
        # def __init__(self, output_dim, input_dim1, hidden_dim1, input_dim2, hidden_dim2, input_dim3, hidden_dim3, hidden_layers, y_dim, latent_dim, act_type, act_args={}):
        self.model = CVAE(p.output_dim, p.input_dim1, p.hidden_dim1, p.input_dim2, p.hidden_dim2, p.input_dim3, p.hidden_dim3, p.hidden_layers, p.y_dim, p.latent_dim, p.act_fn) 
